...
THREE.OculusRiftEffect = function ( renderer, options ) {
	...
	// Orthographic camera
	var oCamera = new THREE.OrthographicCamera( -1, 1, 1, -1, 1, 100000 );
	oCamera.position.z = 1;
	
//	this.cameraMax=(options&&options.cameraMax)?options.cameraMax:10000
	...
	var RTMaterial = new THREE.ShaderMaterial( {
		...
		fragmentShader: useRiftDistortion ? ([
			"uniform vec2 scale;",
			...
			"void main()",
			"{",
  			"gl_FragColor = vec4(texture2D(texid, vUv).b, texture2D(texid, vUv).r, texture2D(texid, vUv).g, 1.0);",
			"}"
		].join("\n") )
	} );
	...
	this.setSize = function ( width, height ) {
		console.log(width)
		console.log(height)
		console.log(HMD.hResolution)
		console.log(HMD.vResolution)
	...
	this.render = function ( scene, camera ) {
		...
		renderer.clearTarget(renderTarget, renderer.autoClearColor, renderer.autoClearDepth, renderer.autoClearStencil);
